...
type DiagramType = "flowchart" | "sequence" | "pie" | "gantt" | "journey";
type NodeShape = "process" | "decision" | "terminator";
type MessageLine = "solid" | "dashed";
type FlowchartArrow = "arrow" | "none" | "dashed" | "thick" | "double";
type GanttStatus = "done" | "active" | "crit" | "";
...
const DEFAULT_NODE: Omit<FlowchartNode, "id" | "label" | "x" | "y"> = {
  width: 160,
  height: 64,
  shape: "process",
  fill: "#F2F2F2",
...
const DEFAULT_EDGE: Omit<FlowchartEdge, "id" | "from" | "to"> = {
  label: "",
  stroke: "#444444",
  arrow: "arrow"
};

const DEFAULT_MESSAGE: Omit<SequenceMessage, "id" | "from" | "to"> = {
  label: "",
  line: "solid"
};

const DEFAULT_PIE_SLICE: Omit<PieSlice, "id"> = {
  label: "Slice",
  value: 20,
  color: "#60a5fa"
};

const DEFAULT_GANTT_TASK: Omit<GanttTask, "id"> = {
  section: "Phase",
  label: "Task",
  start: "2024-01-01",
  end: "2024-01-05",
...
const DEFAULT_JOURNEY_STEP: Omit<JourneyStep, "id"> = {
  section: "Stage",
  task: "Step",
  score: 3,
  personas: ["User"]
...
export function activate(context: vscode.ExtensionContext) {
  const panelState: { panels: { id: number; panel: vscode.WebviewPanel }[] } = {
    panels: []
  };
  const activeWebviews = new Set<vscode.Webview>();
  let diagramState: Diagram | null = null;
  let panelCounter = 1;
  let sidebarWebview: vscode.Webview | null = null;
  let activeMermaidFile: vscode.Uri | null = null;
  ...
  const getDiagram = (root: string): Diagram => {
    if (!diagramState) {
  ...
  const postDiagramToWebview = (webview: vscode.Webview, diagram: Diagram) => {
    try {
      webview.postMessage({
        type: "loadDiagram",
        diagram,
  ...
  const broadcastDiagram = (diagram: Diagram) => {
  ...
  const toWorkspacePath = (root: string, uri: vscode.Uri) =>
    path.relative(root, uri.fsPath).replace(/\\/g, "/");

  const updateSidebarFiles = async (root: string) => {
  ...
  const openMermaidUri = async (root: string, targetUri: vscode.Uri) => {
  ...
  const exportLucid = async (root: string) => {
    ...
    if (!content) {
      vscode.window.showWarningMessage(
        `Lucid CSV export is not available for ${diagram.type} diagrams.`
      );
      return;
    }
  ...
  const initializeWebview = (webview: vscode.Webview, root: string) => {
    webview.options = {
      enableScripts: true,
  ...
  const openEditorPanel = (root: string, options?: { reuse?: boolean }) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mermaidMagic.openEditor", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mermaidMagic.openText", async () => {
      const root = resolveRoot();
      if (!root) return;
      const diagram = getDiagram(root);
      await openMermaidText(diagram);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mermaidMagic.importMermaid", async () => {
      const root = resolveRoot();
      if (!root) return;
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument((document) => {
      if (document.languageId !== "mermaid") return;
      const content = document.getText();
      if (!hasMagicMarker(content)) return;
      const root = getWorkspaceRoot();
      if (!root) return;
      const current = getDiagram(root);
      const updated = parseMermaidToDiagram(content, current);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mermaidMagic.exportMermaid", async () => {
      const root = resolveRoot();
      if (!root) return;
      const diagram = getDiagram(root);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mermaidMagic.exportLucidCsv", async () => {
      const root = resolveRoot();
      if (!root) return;
      await exportLucid(root);
    })
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("mermaidMagic.sidebar", {
...
function dataFilePath(root: string): string {
  return path.join(root, DATA_FILE);
}

function loadDiagram(root: string): Diagram {
...
function saveDiagram(root: string, diagram: Diagram) {
  const filePath = dataFilePath(root);
...
function normalizeDiagram(raw: any): Diagram {
  if (!raw || typeof raw !== "object") {
    return createDefaultDiagram();
  }

  const isLegacy = Array.isArray(raw?.nodes) || Array.isArray(raw?.edges);
  const flowchartNodes: FlowchartNode[] = isLegacy
    ? raw.nodes?.map(normalizeNode) ?? []
    : raw.flowchart?.nodes?.map(normalizeNode) ?? [];
  const flowchartEdges: FlowchartEdge[] = isLegacy
    ? raw.edges?.map(normalizeEdge) ?? []
    : raw.flowchart?.edges?.map(normalizeEdge) ?? [];

  const sequenceParticipants: SequenceParticipant[] = Array.isArray(raw?.sequence?.participants)
    ? raw.sequence.participants.map(normalizeParticipant)
    : [];
  const sequenceMessages: SequenceMessage[] = Array.isArray(raw?.sequence?.messages)
    ? raw.sequence.messages.map(normalizeMessage)
    : [];

  const pie: PieChart = normalizePie(raw?.pie);
  const gantt: GanttChart = normalizeGantt(raw?.gantt);
  const journey: JourneyChart = normalizeJourney(raw?.journey);
...
function normalizeNode(node: any): FlowchartNode {
  return {
    id: Number(node?.id) || 0,
...
function normalizeEdge(edge: any): FlowchartEdge {
  return {
    id: Number(edge?.id) || 0,
...
function normalizeParticipant(participant: any): SequenceParticipant {
  return {
...
function normalizeMessage(message: any): SequenceMessage {
  return {
...
function normalizePie(raw: any): PieChart {
  const slices: PieSlice[] = Array.isArray(raw?.slices)
...
function normalizeGantt(raw: any): GanttChart {
  const tasks: GanttTask[] = Array.isArray(raw?.tasks)
...
function normalizeJourney(raw: any): JourneyChart {
  const steps: JourneyStep[] = Array.isArray(raw?.steps)
    ? raw.steps.map((step: any) => ({
        ...
        personas: Array.isArray(step?.personas)
          ? step.personas.map((persona: any) => String(persona))
...
function isNodeShape(value: any): value is NodeShape {
  return value === "process" || value === "decision" || value === "terminator";
}

function isGanttStatus(value: any): value is GanttStatus {
  return value === "done" || value === "active" || value === "crit" || value === "";
}

function isFlowchartArrow(value: any): value is FlowchartArrow {
  return (
    value === "arrow" ||
    value === "none" ||
    value === "dashed" ||
...
function arrowToMermaid(arrow: FlowchartArrow): string {
  switch (arrow) {
    case "none":
      return "---";
    case "dashed":
...
function arrowFromMermaid(token: string): FlowchartArrow {
  switch (token) {
    case "---":
      return "none";
    case "-.->":
...
function diagramToMermaidFlowchart(flowchart: Diagram["flowchart"]): string {
  const lines: string[] = [];
  ...
  for (const node of flowchart.nodes) {
    ...
    if (!id) continue;
    lines.push(
      `  style ${id} fill:${node.fill},stroke:${node.stroke},color:${node.textColor}`
    );
  }
...
function diagramToMermaidSequence(sequence: Diagram["sequence"]): string {
  const lines: string[] = [];
...
function diagramToMermaidPie(pie: PieChart): string {
  const lines: string[] = [];
  lines.push("pie");
  if (pie.title) {
...
function diagramToMermaidGantt(gantt: GanttChart): string {
  const lines: string[] = [];
  lines.push("gantt");
  if (gantt.title) {
...
function diagramToMermaidJourney(journey: JourneyChart): string {
  const lines: string[] = [];
  lines.push("journey");
  if (journey.title) {
...
function nodeShapeSyntax(shape: NodeShape, label: string): string {
  switch (shape) {
    case "decision":
      return `{${label}}`;
    case "terminator":
...
function diagramToLucidCsv(diagram: Diagram): string | null {
  switch (diagram.type) {
    case "flowchart":
...
function diagramToLucidCsvFlowchart(flowchart: Diagram["flowchart"]): string {
  ...
  const nodeName = (shape: NodeShape) => {
    switch (shape) {
      case "decision":
        return "Decision";
      case "terminator":
...
function diagramToLucidCsvSequence(sequence: Diagram["sequence"]): string {
...
function diagramToLucidCsvPie(pie: PieChart): string {
...
function diagramToLucidCsvGantt(gantt: GanttChart): string {
...
function diagramToLucidCsvJourney(journey: JourneyChart): string {
...
function buildLucidRow(headers: string[], values: Record<string, string>): string[] {
  const row = createLucidRow(headers);
...
function createLucidRow(headers: string[]): string[] {
  return headers.map(() => "");
}

function setLucidColumn(
  row: string[],
  headers: string[],
  column: string,
  value: string,
...
function mapGanttStatus(status: GanttStatus): string {
  switch (status) {
    case "done":
      return "Done";
    case "active":
...
function hasMagicMarker(content: string): boolean {
...
function stripMagicMarker(content: string): string {
  const lines = content.split(/\r?\n/);
...
function parseMermaidToDiagram(content: string, previous: Diagram): Diagram | null {
...
function parseFlowchartDiagram(lines: string[]): { flowchart: Diagram["flowchart"] } {
  const nodes = new Map<string, FlowchartNode>();
  const styles = new Map<string, Partial<FlowchartNode>>();
  let nextId = 1;

  const allocateId = (token: string) => {
  ...
  const ensureNode = (token: string, label?: string, shape?: NodeShape) => {
    const existing = nodes.get(token);
    if (existing) {
  ...
  for (const line of lines) {
    ...
    if (styleMatch) {
      ...
      for (const part of styleParts) {
        ...
        if (key === "fill") updates.fill = value;
        if (key === "stroke") updates.stroke = value;
        if (key === "color") updates.textColor = value;
      }
  ...
  for (const [token, update] of styles.entries()) {
    ...
    if (!node) continue;
    if (update.fill) node.fill = update.fill;
    if (update.stroke) node.stroke = update.stroke;
    if (update.textColor) node.textColor = update.textColor;
  }

  return { flowchart: { nodes: nodeList, edges } };
}

function parsePieDiagram(lines: string[]): { pie: PieChart } {
  let title = "";
...
function parseGanttDiagram(lines: string[]): { gantt: GanttChart } {
  let title = "";
...
function parseJourneyDiagram(lines: string[]): { journey: JourneyChart } {
  let title = "";
...
function parseSequenceDiagram(lines: string[]): { sequence: Diagram["sequence"] } {
  const participants: SequenceParticipant[] = [];
  const messages: SequenceMessage[] = [];
  const aliasToId = new Map<string, number>();
  let nextId = 1;

  const ensureParticipant = (alias: string, name?: string) => {
    if (aliasToId.has(alias)) return aliasToId.get(alias)!;
...
function computeNextId(diagram: Diagram, fallback: number): number {
...
async function openMermaidText(diagram: Diagram) {
...
function getNoWorkspaceHtml(): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
